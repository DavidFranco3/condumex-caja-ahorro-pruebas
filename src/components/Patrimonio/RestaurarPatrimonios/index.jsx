...
const RestaurarPatrimonios = ({ setShowModal, history }) => {
    ...
    const handleSubmit = async (evt) => {
        ...
        setLoading(true);
        for (const { fichaSocio, patrimonio, createdAt } of dataFile) {
            ...
            await registraPatrimonio(dataAportacion);
        ...
        if (files.length > 0) {
          ...
          reader.onload = (evt) => {
            ...
            const data = lines.map(line => {
                const [fichaSocio, patrimonio, createdAt] = line.split(',');
                return { fichaSocio, patrimonio, createdAt}
            });
            setDataFile(data.filter(({ fichaSocio, patrimonio, createdAt }) => fichaSocio && patrimonio && createdAt));
          };
...
export default RestaurarPatrimonios;