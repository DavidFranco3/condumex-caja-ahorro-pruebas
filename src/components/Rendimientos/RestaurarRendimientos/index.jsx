...
const RestaurarRendimientos = ({ setShowModal, history }) => {
    ...
    const handleSubmit = async (evt) => {
        ...
        setLoading(true);
        for (const { fichaSocio, rendimiento, createdAt } of dataFile) {
            ...
            // Registra Saldos
            await registroSaldoInicial(fichaSocio, "0", "0", rendimiento, folio, "Interés");
        ...
        if (files.length > 0) {
            ...
            reader.onload = (evt) => {
                ...
                const data = lines.map(line => {
                    const [fichaSocio, rendimiento, createdAt] = line.split(',');
                    return { fichaSocio, rendimiento, createdAt }
                });
                setDataFile(data.filter(({ fichaSocio, rendimiento, createdAt }) => fichaSocio && rendimiento && createdAt));
            };
...
export default RestaurarRendimientos;