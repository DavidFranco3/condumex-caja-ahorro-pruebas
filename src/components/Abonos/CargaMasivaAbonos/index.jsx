...
import { Button, Col, Form, Row, Spinner, InputGroup, ProgressBar } from 'react-bootstrap';
...
const razonSocial = getRazonSocial();
    ...
    await registraAbonos(dataAbonos);
        ...
        await actualizacionDeudaSocio(parseInt(fichaSocio), abono, "0", "Abono", formData.fecha);
        
        await registroMovimientosSaldosSocios(fichaSocio, "0", "0", "0", "0", "0", "0", abono, "Abono");
        
        await registroDeudaSocioInicial(parseInt(fichaSocio), abono, "0", "Abono", formData.fecha);
...
const { result } = evt.target;
        ...
        const [fichaSocio, abono] = line.split('\t');
                return { fichaSocio, abono }

        });
        setDataFile(data.filter(({ fichaSocio, abono }) => fichaSocio && abono));
}
...
export default CargaMasivaAbonos;
