...
const CargaMasivaPrestamos = ({ setShowModal, history }) => {
    ...
    const handleSubmit = async (evt) => {
        ...
        for (const { fichaSocio, prestamo } of dataFile) {
            console.log("la ficha del socio es: ", fichaSocio)
        ...
        if (files.length > 0) {
            ...
            reader.onload = (evt) => {
                ...
                const data = lines.map(line => {
                    const [fichaSocio, prestamo] = line.split('\t');
                    return { fichaSocio, prestamo }

                });
                setDataFile(data.filter(({ fichaSocio, prestamo }) => fichaSocio && prestamo));
            }
...
export default CargaMasivaPrestamos;
