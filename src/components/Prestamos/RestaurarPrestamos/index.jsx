...
const RestaurarPrestamos = ({ setShowModal, history }) => {
    ...
    const handleSubmit = async (evt) => {
        ...
        setLoading(true);
        for (const { fichaSocio, prestamo, createdAt } of dataFile) {
            const fecha = createdAt.split("T");
            console.log(new Date (fecha[0]))
            const responseFolio = await obtenerFolioActualPrestamo();
        ...
        if (files.length > 0) {
          ...
          reader.onload = (evt) => {
            ...
            const data = lines.map(line => {
                const [fichaSocio, prestamo, createdAt] = line.split(',');
                return { fichaSocio, prestamo, createdAt}
            });
            setDataFile(data.filter(({ fichaSocio, prestamo, createdAt }) => fichaSocio && prestamo && createdAt));
          };
...
export default RestaurarPrestamos;